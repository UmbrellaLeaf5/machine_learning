...
def relu_backward(dout, cache):
  ...
  dx = dout * (x > 0)  # градиент: 1 если x>0, иначе 0
...
def dropout_forward(x, dropout_param):
  ...
  """
  _p, mode = dropout_param["p"], dropout_param["mode"]
    ...
    for f in range(F):
      for i in range(H_out):
        for j in range(W_out):
          ...
          x_region = x_pad[n, :, h_start:h_end, w_start:w_end]
    ...
    for f in range(F):
      for i in range(H_out):
        for j in range(W_out):
          ...
          dx_pad[n, :, h_start:h_end, w_start:w_end] += d * w[f]
...
def max_pool_forward_naive(x, pool_param):
  ...
  for i in range(H_out):
    for j in range(W_out):
      ...
      # Берем окно и находим максимум по осям высоты и ширины
...
def max_pool_backward_naive(dout, cache):
  ...
  for i in range(H_out):
    for j in range(W_out):
      ...
      window_flat = window.reshape(N, C, pool_height * pool_width)
      max_indices = np.argmax(window_flat, axis=2)

      mask = np.zeros_like(window_flat)
      for n in range(N):
        for c in range(C):
          mask[n, c, max_indices[n, c]] = 1

      mask = mask.reshape(N, C, pool_height, pool_width)

      dx_window = mask * dout[:, :, i, j].reshape(N, C, 1, 1)

      dx[:, :, h_start:h_end, w_start:w_end] += dx_window
